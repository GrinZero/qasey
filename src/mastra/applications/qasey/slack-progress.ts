...
function renderToolCall(toolName: string, args: unknown, taskTopic?: string): string | undefined {
  ...
  if (toolName.startsWith("case_hub_")) {
    if (toolName === "case_hub_create_review_plan") return "正在整理待审文字用例…";
    if (toolName === "case_hub_start_e2e") return "正在为已批准版本启动 E2E 验证…";
    if (toolName === "case_hub_create_change_set") return "旧的直启入口已停用，正在要求先审核文字用例…";
...
function publicToolTitle(toolName: string): string {
  ...
  if (toolName === "case_hub_create_review_plan") return "创建文字用例审核";
  if (toolName === "case_hub_start_e2e") return "启动 E2E 交付";
  if (toolName === "case_hub_create_change_set") return "旧入口已停用";
...
function renderToolResult(
  ...
  if (toolName.startsWith("case_hub_")) {
    if (toolName === "case_hub_create_review_plan") return "文字用例计划已创建，等待用户审核。";
    if (toolName === "case_hub_start_e2e") return "E2E 运行已启动，正在隔离环境中生成和验证 Playwright…";
    if (toolName === "case_hub_create_change_set") return "旧的直启入口已停用，请先审核文字用例。";
