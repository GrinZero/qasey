import { QaseyE2EContextSchema } from "../../../../packages/contracts/src/index.ts";
import {
...
export function conversationTurnsToUIMessages(
  ...
  return turns.flatMap(turn => {
    ...
    const metadata = { ...messageMetadata(turn, latestSequence), ...e2eMetadata(events) };
    ...
    for (const event of events) {
      ...
      } else if (event.type === "review-plan.linked") {
        const data = reviewPlanData(event);
        if (data) assistantParts.push({ type: "data-case-review", id: `${turn.id}:case-review:${data.planId}`, data });
...
export function conversationEventStreamResponse(input: {
  ...
  const stream = createUIMessageStream<QaseyUIMessage>({
    ...
    execute: async ({ writer }) => {
      const accepted = await input.repository.events(input.owner, input.subjectId, input.conversationId, input.turn.id, 0);
      const contextMetadata = e2eMetadata(accepted);
      ...
      writer.write({
        ...
        messageMetadata: { ...messageMetadata(input.turn, cursor), ...contextMetadata },
      });
      ...
      while (!input.signal.aborted) {
        ...
        for (const event of events) {
          ...
          if (event.type === "assistant.delta") {
            ...
            const data = reviewPlanData(event);
            if (data) writer.write({ type: "data-case-review", id: `${input.turn.id}:case-review:${data.planId}`, data });
          } else if (event.type === "failed") {
...
function reviewPlanData(event: QaseyConversationEvent): QaseyCaseReviewData | undefined {
  const planId = stringPayload(event, "planId");
  const status = stringPayload(event, "status");
  const revision = numberPayload(event, "revision");
  if (!planId || !revision || status !== "reviewing" && status !== "ready" && status !== "cancelled") return undefined;
  return {
    planId, revision, status,
    pendingCount: numberPayload(event, "pendingCount") ?? 0,
    approvedCount: numberPayload(event, "approvedCount") ?? 0,
    removedCount: numberPayload(event, "removedCount") ?? 0,
  };
}

function numberPayload(event: QaseyConversationEvent, key: string): number | undefined {
  const value = event.payload[key];
...
function e2eMetadata(events: QaseyConversationEvent[]): Pick<QaseyUIMessageMetadata, "e2eContext"> {
  const parsed = QaseyE2EContextSchema.safeParse(events.find(event => event.type === "accepted")?.payload.e2eContext);
  return parsed.success ? { e2eContext: parsed.data } : {};
}
