...
const authorAndPersistPatch = createStep({
  ...
  execute: async ({ inputData }) => {
    try {
      await e2eCoordinator.authorAndPersistPatch(inputData.runId, inputData.qaFeedback);
    } catch (error) {
      await e2eCoordinator.fail(inputData.runId, error);
      throw error;
    }
...
const cleanVerifyAndPublish = createStep({
  ...
  execute: async ({ inputData }) => {
    try {
      await e2eCoordinator.cleanVerifyAndPublish(inputData.runId);
    } catch (error) {
      await e2eCoordinator.fail(inputData.runId, error);
      throw error;
    }
    const run = await runRepository.get(inputData.runId);
...
export const awaitQaVerdictStep = createStep({
  ...
  execute: async ({ inputData, resumeData, suspend }) => {
    ...
    const updated = await e2eCoordinator.verdict(inputData.runId, resumeData);
    if (resumeData.verdict === "approve") return { runId: updated.id, status: updated.status };

    try {
      await e2eCoordinator.authorAndPersistPatch(inputData.runId, resumeData.feedback);
      await e2eCoordinator.cleanVerifyAndPublish(inputData.runId);
    } catch (error) {
      await e2eCoordinator.fail(inputData.runId, error);
      throw error;
    }
    const repaired = await runRepository.get(inputData.runId);
...
export const e2eLifecycleWorkflow = createWorkflow({
  ...
  options: {
    onError: async ({ runId, error }) => {
      await e2eCoordinator.fail(runId, error?.message ?? "E2E workflow failed").catch(() => undefined);
    },
  },
})
...
export async function createAndStartE2ERun(
  ...
  const workflow = mastra.getWorkflow("e2eLifecycleWorkflow");
  const run = await workflow.createRun({ runId: created.id, ...(resourceId ? { resourceId } : {}) });
  await run.startAsync({ inputData: { runId: created.id } });
  return created;
}

export async function rerunE2E(mastra: Mastra, runId: string, resourceId?: string): Promise<E2ERun> {
...
export async function resumeE2EWithVerdict(mastra: Mastra, runId: string, verdict: QaVerdict): Promise<E2ERun> {
  if (!config.QASEY_ENABLE_EXECUTION) return e2eCoordinator.verdict(runId, verdict);
  const workflow = mastra.getWorkflow("e2eLifecycleWorkflow");
  const run = await workflow.createRun({ runId });
  await run.resume({ step: awaitQaVerdictStep, resumeData: verdict });
  const updated = await runRepository.get(runId);
  if (!updated) throw new Error(`Run ${runId} not found after workflow resume`);
  return updated;
}

export async function cancelE2ERun(mastra: Mastra, runId: string): Promise<E2ERun> {
  if (config.QASEY_ENABLE_EXECUTION) {
    const workflow = mastra.getWorkflow("e2eLifecycleWorkflow");
    const run = await workflow.createRun({ runId });
    await run.cancel().catch(() => undefined);
  }
  return e2eCoordinator.cancel(runId);
}
