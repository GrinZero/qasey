...
export function assertWebE2EAutomationPaths(
  ...
  for (const proposal of proposals) {
    if (!proposal.automationPath) {
      throw new Error("Legacy E2E proposals require automationPath; text review proposals never accept it");
    }
    const automationPath = proposal.automationPath;
    const project = policy.projects.find(candidate => isWithin(automationPath, candidate.testRoot));
    if (!project) {
      throw new Error(`Case automationPath is outside the configured Playwright test roots: ${proposal.automationPath}`);
    }
    if (!project.testFileSuffixes.some(suffix => automationPath.endsWith(suffix))) {
      throw new Error(
