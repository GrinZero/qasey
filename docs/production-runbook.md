...
2. Run `pnpm check` and confirm both `.mastra/output/index.mjs` and `.mastra/worker/index.mjs` exist. Smoke OAuth, `/admin`, Agent generate/stream, Workflow start/suspend/resume, Run/Artifact access, Slack mention/DM/thread/attachment/approval, Jira, MCP, Workspace, and audit.
...
3. Provision the dedicated `qasey.mastra.worker.token` secret, shared PostgreSQL, and Redis Streams connection. Deploy the API with `MASTRA_WORKERS=false`, then the private orchestration Worker with `MASTRA_WORKERS=orchestration`, `MASTRA_STEP_EXECUTION_URL=http://moego-qasey-api/studio/api`, and `MASTRA_WORKER_AUTH_TOKEN`. Production Studio must remain disabled.
