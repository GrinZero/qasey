# Qasey QA Agent 近期规划与实施路线图

> 当前状态：M1/M2 + Web/App E2E vertical-slice implemented（组织环境接入与生产 runner pool 待配置）
> 原技术方案制定：2026-08-13
> 最近更新：2026-08-18
> 文档用途：用 OGSM 对齐近期目标与验收口径，并持续维护代码化迁移、E2E 左移和生产化实施细节。

## 0. 近期 OGSM 总纲

### Objective（定性目标）

让 Qasey 成为**可实际使用的 QA Agent**：能够稳定地把真实需求输入转化为可评审的测试用例，并进一步生成、执行和验证 E2E 测试；全过程可追踪、可人工介入、可持续优化。

### Goal（阶段目标）

| 阶段 | 截止时间 | 可衡量目标 | 完成标准 |
|---|---|---|---|
| G1：代码版具备可用的生产化基础 | 本周（2026-08-17 ～ 2026-08-23） | 完成 Code 版本上云；完善 Sandbox/CUA 基础能力；启动 `Case → E2E`；建立 QA 历史会话评测集和优化闭环 | 代码版可在云端处理至少 1 条真实 QA 链路；Sandbox 可隔离运行并回收日志/截图等证据；`Case → E2E` 完成首个端到端样例；历史会话已形成可重复运行的基线评测集 |
| G2：核心链路进入实际试用 | 下周（2026-08-24 ～ 2026-08-30） | 完成 `Case → E2E` 第一版，并将 `Input → Case` 推进到可实际使用 | 在约定评测集上：`Input → Case` 成功率 ≥ 85%、Case 人工修改率 ≤ 30%；`Case → E2E` 生成成功率 ≥ 70%、稳定环境下 E2E 执行成功率 ≥ 60%；Agent 任务完成率 ≥ 70%、平均人工介入次数 ≤ 2 次/任务 |

上述数值是近期试用阶段的初始门槛。每周复盘时根据样本规模、失败分类和真实使用反馈调整，但调整必须记录原因，不能通过缩小样本范围或排除失败样本来达标。

### Strategy（关键路径）

#### S1：打通 `Input → Case → E2E` 核心链路

- 统一需求输入、测试分析、`TestCaseSpec` 和 E2E 生成之间的数据契约。
- 先把 `Input → Case` 做到真实 QA 可以直接试用，再以结构化 Case 驱动 Playwright/Maestro 代码生成。
- 建立 Author → 有界修复 → Clean-room Verifier → Evidence 的闭环，以真实执行结果而非模型自述判断成功。
- 优先覆盖少量高频 P0 场景，确保链路稳定后再扩展场景和平台矩阵。

#### S2：完善 Sandbox + CUA 执行能力

- 为每个任务提供隔离、可写、可执行、可回收证据的 Sandbox，并限制权限、资源、时长和网络访问。
- 明确 Author 与 Verifier 环境边界，避免脏环境、测试代码自我迎合和 false green。
- CUA 仅用于探索、系统 UI、跨应用和定位失败兜底；结果必须沉淀为 Playwright/Maestro 代码或交由 QA 判断，不直接作为测试通过证据。

#### S3：基于 QA 历史会话持续优化 Agent

- 从 QA 历史会话提取代表性输入、期望 Case、人工修改和失败原因，形成脱敏、可版本化的 golden dataset。
- 每次 Prompt、模型、工具、工作流或数据契约变更后运行回归评测，并与当前基线比较。
- 对失败按上下文缺失、事实误判、覆盖不足、格式/契约错误、环境问题和执行问题分类，优先修复高频且高影响问题。
- 将 QA 的修改和反馈回流为评测样本；未经评测验证的变化不直接扩大流量。

#### S4：以代码上云、可观测和安全切流支撑真实使用

- 完成代码版部署、配置、队列、状态持久化、日志/trace 和告警的最小生产闭环。
- 保持外部写操作幂等，按 Shadow → Canary → 全量逐步切流，并保留 n8n 回滚路径。
- 以真实任务的成功率、耗时、人工介入和失败分类决定放量，而不是只以功能开发完成度判断可用性。

### Measurement（过程指标）

| 指标 | 计算口径 | 下周阶段目标 | 主要数据来源 |
|---|---|---:|---|
| `Input → Case` 成功率 | 通过 schema 校验、满足必填信息且可进入 QA 评审的 Case 任务数 ÷ `Input → Case` 总任务数 | ≥ 85% | Workflow 状态、schema 校验、QA 评审结果 |
| Case 人工修改率 | QA 认为需要实质修改的 Case 数 ÷ 已生成并评审的 Case 总数；仅排版调整不计入实质修改 | ≤ 30% | QA Review、Case diff、反馈标签 |
| `Case → E2E` 生成成功率 | 生成可被目标框架解析、编译/加载并进入执行阶段的 E2E 任务数 ÷ 发起生成的任务总数 | ≥ 70% | Generator、编译/加载结果、Runner 记录 |
| E2E 执行成功率 | 在已确认环境稳定的样本中，Clean-room Verifier 按预期完成且断言通过的任务数 ÷ 实际执行任务数 | ≥ 60% | Verifier 退出码、断言、Evidence Manifest |
| Agent 任务完成率 | 达到任务定义终态且交付物完整的任务数 ÷ 已受理任务总数；取消和环境故障单独标注，不静默剔除 | ≥ 70% | Run 状态机、Artifact 完整性、失败分类 |
| 平均人工介入次数 | 所有受理任务中的人工补充、纠错、批准重试次数之和 ÷ 已受理任务数 | ≤ 2 次/任务 | HITL 事件、QA feedback、重试记录 |

为避免指标失真，周报需同时记录样本量、目标仓库/平台、环境故障占比和 P50/P95 耗时。E2E 执行失败必须区分产品缺陷、生成缺陷、测试环境故障和 flaky，不能用自动重试后的结果覆盖首次失败。

### 本周与下周执行重点

#### 本周（2026-08-17 ～ 2026-08-23）

- [ ] 完成 Code 版本上云，验证配置、数据库、队列、日志/trace 和最小告警链路。
- [ ] 完善 Sandbox/CUA：隔离工作区、执行限额、证据回收、失败分类和受控兜底策略可用。
- [ ] 启动 `Case → E2E`，跑通至少 1 个真实 Case 的生成、执行、证据回收 vertical slice。
- [ ] 整理并脱敏 QA 历史会话，建立带期望结果和失败标签的首版评测集。
- [ ] 记录六项核心指标的当前基线，为下周验收提供可比数据。

#### 下周（2026-08-24 ～ 2026-08-30）

- [ ] 完成 `Case → E2E` 第一版，支持约定的首批目标仓库/平台并输出可复现的执行证据。
- [ ] 将 `Input → Case` 推进到可实际使用，邀请真实 QA 在 P0 场景试用并记录反馈。
- [ ] 在固定评测集上验收 G2 的六项指标，输出失败分类、样本量和与基线的差异。
- [ ] 将历史会话与试用反馈纳入回归集，形成“变更 → 评测 → 小流量验证 → 复盘”的持续优化节奏。

### 维护约定

- 每周一更新本周目标、负责人和样本范围；每周五更新实际值、失败分类和下周动作。
- 完成事项勾选后保留一周，并补充结果或证据链接；长期有效的结论沉淀到后续技术章节。
- 指标口径或目标值变化时，在本节记录变更日期、原因和影响，避免历史数据不可比。
- 新增工作必须关联至少一个 Goal 或 Strategy；无法关联的事项不进入近期 P0/P1。
