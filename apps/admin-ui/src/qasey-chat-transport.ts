    ...
    super({
      ...
      prepareSendMessagesRequest: ({ messages, body }) => {
        ...
        if (action && typeof action === "object") {
          return { api: `${conversationApi}/actions`, body: { ...action, clientMessageId: latest.id } };
        }
...
function existingMessageChunks(message: QaseyUIMessage): ReadableStream<UIMessageChunk> {
  ...
  message.parts.forEach((part, index) => {
    ...
    } else if (part.type === "data-progress" || part.type === "data-run" || part.type === "data-cursor" || part.type === "data-case-review") {
      chunks.push(part);
    }
  });
