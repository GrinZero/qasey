...
describe("Qasey AI SDK UI message projection", () => {
  it("projects persisted turns with stable message and data-part ids", async () => {
    ...
    await repository.appendEvent(owner, "qa-user", conversation.id, started.turn.id, "review-plan.linked", {
      planId, revision: 1, status: "reviewing", pendingCount: 2, approvedCount: 0, removedCount: 0,
    });
    ...
    expect(messages[1]?.metadata?.latestSequence).toBe(7);
    ...
    expect(messages[1]?.parts).toContainEqual({
      type: "data-case-review",
      id: `${turn.id}:case-review:${planId}`,
      data: { planId, revision: 1, status: "reviewing", pendingCount: 2, approvedCount: 0, removedCount: 0 },
    });
    expect(messages[1]?.parts).toContainEqual({
      type: "dynamic-tool",
      toolCallId: "github-call-1",
  ...
  it("streams only events after the cursor as AI SDK v7 UI chunks", async () => {
    const repository = new InMemoryQaseyConversationRepository(() => new Date("2026-09-04T04:00:00.000Z"));
    const conversation = await repository.createConversation(owner, "qa-user");
    const started = await repository.startTurn(
      owner,
