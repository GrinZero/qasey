...
describe("E2E fixture lease service", () => {
  it("checks the deployed SHA, creates an isolated lease, and cleans it up directly", async () => {
    ...
    const service = new E2EFixtureLeaseService({ schemaVersion: 1, sourceSha }, "https://e2e.example.test/");
    const lease = await service.acquire({ owner: { applicationId: "qasey", tenantId: "tenant-1" }, runId: "run-1", expectedSourceSha: sourceSha });
    expect(lease).toMatchObject({ baseUrl: "https://e2e.example.test", sourceSha });
    expect(lease.id).toBeTruthy();
    expect(lease.sessionToken).toHaveLength(43);
    await service.release(lease);
    await service.release(lease);
  });

  it("blocks an environment source mismatch before allocating a lease", async () => {
    const service = new E2EFixtureLeaseService({ schemaVersion: 1, sourceSha: "b".repeat(40) }, "https://e2e.example.test");
    await expect(service.acquire({ owner: { applicationId: "qasey", tenantId: "tenant-1" }, runId: "run-1", expectedSourceSha: "a".repeat(40) }))
      .rejects.toThrow("environment_version_mismatch");
  });

  it("enforces ownership for the service-only cleanup API", async () => {
    const service = new E2EFixtureLeaseService({ schemaVersion: 1, sourceSha: "a".repeat(40) }, "https://e2e.example.test");
