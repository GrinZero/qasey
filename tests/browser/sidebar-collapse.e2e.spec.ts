...
test(
  ...
  async ({ page }) => {
    ...
    await test.step("Step 01 · 打开 /admin/apps/qasey，检查桌面侧栏和顶部的侧栏切换按钮。", async () => {
      ...
      await expect(sidebar(page)).toHaveCSS("width", "238px");
      await expect(mainArea(page)).toHaveCSS("margin-left", "238px");
    ...
    await test.step("Step 02 · 点击“收起侧边栏”。", async () => {
      ...
      const qaseyApplication = mainNavigation(page).getByRole("button", { name: "Qasey", exact: true });
      await expect(sidebar(page)).toHaveCSS("width", "76px");
      await expect(mainArea(page)).toHaveCSS("margin-left", "76px");
