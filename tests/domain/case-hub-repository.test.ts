...
describe("Case Hub repository", () => {
  ...
  it("maps request_changes verdicts to the persisted changes_requested status", async () => {
    ...
    const repeated = await hub.reviewResult(owner, result!.id, "reviewer-1", {
      verdict: "request_changes",
      feedback: "补充稳定的滚动压力数据",
    });
    expect(repeated).toEqual(reviewed);
    await expect(hub.reviewResult(owner, result!.id, "reviewer-1", {
      verdict: "request_changes",
      feedback: "换一个反馈不属于同一次幂等重试",
    })).rejects.toThrow("Only pending results can be reviewed");
  });
