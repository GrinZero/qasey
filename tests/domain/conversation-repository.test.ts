...
describe("Qasey conversation repository", () => {
  ...
  it("deduplicates messages, allows overlapping turns, and preserves each turn lifecycle", async () => {
    ...
    const concurrent = await repository.startTurn(
      owner, "qa-1", conversation.id, "22222222-2222-4222-8222-222222222222", "并发消息",
    );
    expect(concurrent.created).toBe(true);
    ...
    expect((await repository.getConversation(owner, "qa-1", conversation.id))?.activeTurnId).toBe(concurrent.turn.id);
    await repository.appendEvent(owner, "qa-1", conversation.id, concurrent.turn.id, "completed", { text: "独立回复" });
