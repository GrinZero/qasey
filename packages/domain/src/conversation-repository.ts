...
import {
  ...
  QaseyE2EContextSchema,
  type QaseyE2EContext,
...
export interface QaseyConversationRepository {
  ...
  startTurn(owner: OwnerScope, subjectId: string, conversationId: string, clientMessageId: string, message: string, e2eContext?: QaseyE2EContext): Promise<StartedConversationTurn>;
...
export class InMemoryQaseyConversationRepository implements QaseyConversationRepository {
  ...
  async startTurn(owner: OwnerScope, subjectId: string, conversationId: string, clientMessageId: string, message: string, e2eContext?: QaseyE2EContext): Promise<StartedConversationTurn> {
    ...
    const accepted = QaseyConversationEventSchema.parse({
      ...
      payload: { message, ...(e2eContext ? { e2eContext: QaseyE2EContextSchema.parse(e2eContext) } : {}) }, occurredAt: now,
    });
  ...
  async appendEvent(owner: OwnerScope, subjectId: string, conversationId: string, turnId: string, type: ConversationEventType, payload: Record<string, unknown> = {}): Promise<QaseyConversationEvent> {
    ...
    this.conversations.set(conversationKey, QaseyConversationSchema.parse({
      ...conversation, ...(status === "running" || conversation.activeTurnId !== turnId ? {} : { activeTurnId: undefined }), updatedAt: now,
    }));
